...
<body>
  ...
  <div class="part-banner"><div class="t">근무시간 분석</div><div class="s">'근태예외자' 제외 · 제조팀·체계기술2팀은 제주우주센터로 통합 · 1~4주차 평균 기준</div></div>
...
function processData(rows){
  const cleaned = rows.filter(r => !EXCLUDE_구분.includes(String(r['구분']).trim()));
  cleaned.forEach(r=>{
